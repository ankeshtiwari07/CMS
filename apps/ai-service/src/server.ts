...
app.post("/studio/website/plan", async (req, reply) => {
  ...
  try {
    ...
    return { ok: true, title: plan.title, description: plan.description, brand: plan.brand, sections: plan.sections, lang: plan.lang, dir: plan.dir, bilingual: plan.bilingual, provider: plan._provider };
  } catch (e: any) {
...
app.post("/studio/website", async (req, reply) => {
  ...
  try {
    ...
    // Auto-governance: a fast, deterministic brand check runs on EVERY generate
    // (no LLM, no blocking) so an on-brand score surfaces automatically. The deep
    // grounded review remains available on demand via /studio/governance/review.
    const bp = await getBrandPalette().catch(() => ({ palette: [], name: "" }));
    const brandCheck = { ...quickBrandScore(site.html, bp.palette), brandName: bp.name };
    return {
      ok: true, title: site.title, brand: site.brand, sections: site.sections, html: site.html, lang: site.lang, dir: site.dir, provider: site.provider, brandCheck,
      // Component-based build telemetry (LEAP D2): which library components were
      // reused, and which were auto-generated by the Component Agent for gaps.
      // The studio persists `delegatedComponents` to the library and routes them
      // (with the page) through the approval flow (D3 / Flow B).
      usedComponents: site.usedComponents,
      delegatedComponents: site.delegatedComponents,
    };
  } catch (e: any) {
...
app.post("/studio/page", async (req, reply) => {
  ...
  try {
    ...
    const brandCheck = { ...quickBrandScore(site.html, bp.palette), brandName: bp.name };
    return {
      ok: true, title: site.title, contentType: body.contentType, brand: site.brand, blocks, html: site.html, brandCheck,
...
app.post("/studio/component", async (req, reply) => {
  ...
  const brand: any = {
    ...
    line: "#e6ebeb", soft: "#f5f8f7",
    font: "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif", radius: 20,
    ...(body.brand || {}),
  };
  try {
    const c = await generateComponent({ kind: body.kind as any, brief: body.brief, brand, siteTitle: body.siteTitle, primary: entry.provider });
    return { ok: true, component: c, provider: c._provider };
  } catch (e: any) {
...
// Index the active brand corpus into the RAG pipeline (grounds the agent).
app.post("/studio/governance/index-brand", async (_req, reply) => {
  try {
    const r = await indexBrandCorpus();
    return { ok: true, ...r };
  } catch (e: any) {
    return reply.code(502).send({ error: e?.message || "index_failed" });
  }
});

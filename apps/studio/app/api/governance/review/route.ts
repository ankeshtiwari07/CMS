...
// POST /api/governance/review — Governance Agent reviews an artifact against the brand.
export async function POST(req: Request) {
  if (!(await getCurrentUser())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const body = await req.json().catch(() => ({}));
  if (!body?.html) return NextResponse.json({ error: "html is required" }, { status: 400 });
  try {
    const r = await fetch(`${AI_URL}/studio/governance/review`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });

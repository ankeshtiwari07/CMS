...
export default async function DeckStudioPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  const rail: React.CSSProperties = { width: 40, height: 40, borderRadius: 11, display: "grid", placeItems: "center", color: "#5a6a6c" };
  const railActive: React.CSSProperties = { ...rail, background: "#e7f6ef", color: "#00a18b" };

  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#eef4f3" }}>
      <nav style={{ width: 68, flexShrink: 0, background: "#fff", borderInlineEnd: "1px solid #e3ebe9", display: "flex", flexDirection: "column", alignItems: "center", padding: "18px 0", gap: 8 }}>
        ...
        <div style={{ marginTop: "auto", width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#b6f24a,#10b496)", color: "#07110c", display: "grid", placeItems: "center", fontWeight: 800, fontSize: 14 }}>{initial}</div>
      </nav>

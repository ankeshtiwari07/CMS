...
const TEAL_DARK = "var(--studio-teal-dark)";
const LIME = "var(--lime)";
const INK = "var(--ink)";
...
const bar = (w: number | string, c = "var(--hairline)", h = 6): React.CSSProperties => ({
  ...
  return (
    <div style={{ ...frame, background: "linear-gradient(135deg,#eef7f4,#dcefe7)" }}>
      <div style={{ position: "relative", width: "82%" }}>
        {/* back slide peeking */}
        <div style={{ position: "absolute", top: -8, left: 10, right: -10, height: 84, background: "var(--card)", borderRadius: 8, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", opacity: 0.6 }} />
        {/* front slide */}
        <div style={{ position: "relative", background: "var(--card)", borderRadius: 8, padding: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
          ...
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            ...
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--hairline)" }} />
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--hairline)" }} />
          </div>
        </div>
      </div>
...
export function ImageThumb() {
  return (
    <div style={{ ...frame, background: "var(--deep-teal)" }}>
...
export function WebsiteThumb() {
  return (
    <div style={{ ...frame, background: "linear-gradient(135deg,#eaf2ff,#f3ecff)" }}>
      <div style={{ width: "88%", background: "var(--card)", borderRadius: 8, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
        {/* browser chrome */}
        <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 8px", background: "var(--surface-2)", borderBottom: `1px solid ${LINE}` }}>
        ...
        <div style={{ padding: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <div style={{ flex: 1, height: 22, borderRadius: 5, background: "var(--surface-2)" }} />
            <div style={{ flex: 1, height: 22, borderRadius: 5, background: "var(--surface-2)" }} />
            <div style={{ flex: 1, height: 22, borderRadius: 5, background: "var(--surface-2)" }} />
          </div>
        </div>
...
export function EmailThumb() {
  return (
    <div style={{ ...frame, background: "linear-gradient(135deg,var(--surface-2),var(--surface-3))" }}>
      <div style={{ width: "84%", background: "var(--card)", borderRadius: 8, padding: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 9 }}>
          <span style={{ width: 18, height: 18, borderRadius: "50%", background: TEAL, color: "var(--primary-foreground)", fontSize: 9, fontWeight: 700, display: "grid", placeItems: "center" }}>H</span>
...
export function WritingThumb() {
  return (
    <div style={{ ...frame, background: "linear-gradient(135deg,#f5f2ec,#e9e2d6)" }}>
      <div style={{ width: "76%", background: "var(--card)", borderRadius: 8, padding: "14px 14px 16px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
  ...
  return (
    <div style={{ ...frame, background: "linear-gradient(135deg,#fff4d6,#f3dca0)" }}>
      <div style={{ width: "88%", display: "flex", gap: 8 }}>
        <div style={{ flex: 1, background: "var(--card)", borderRadius: 8, padding: 10, boxShadow: "0 2px 6px rgba(0,0,0,0.08)" }}>
        ...
        <div dir="rtl" style={{ flex: 1, background: "var(--card)", borderRadius: 8, padding: 10, boxShadow: "0 2px 6px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: MUTED, marginBottom: 7 }}>ع</div>
          <div style={{ ...bar("100%", "var(--hairline)"), marginBottom: 5 }} />
          <div style={{ ...bar("80%", "var(--hairline)"), marginInlineStart: "auto" }} />
        </div>
      </div>
    </div>
  );

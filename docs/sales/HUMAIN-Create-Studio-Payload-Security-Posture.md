...
| **Access control** | **RBAC (7 roles)** + **ABAC** (site-scope, department, locale) enforced at collection and field level; roles/scope admin-editable only (no self-escalation); `siteAdmin` confined to its sites; **behaviourally verified** by a per-role allow/deny test suite |

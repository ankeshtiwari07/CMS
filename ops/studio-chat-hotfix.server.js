...
app.post("/run", async (req, reply) => {
    const body = z
        ...
        .parse(req.body);
    const entry = prompts[body.promptId];
    if (!entry)
        return reply.code(400).send({ error: "unknown promptId" });
    if (provider.configured === false)
        return reply.code(503).send({ error: "llm_not_configured" });
    let raw;
    let usedProvider = provider.name;
    try {
        const fb = await completeWithFallback({ system: entry.system, messages: [{ role: "user", content: body.input }], fast: body.fast, maxTokens: 1024 }, { primary: process.env.LLM_PROVIDER || "anthropic" });
        raw = fb.text;
        usedProvider = fb.provider;
        if (fb.fellBack)
            req.log.warn({ tried: fb.tried, used: fb.provider, promptId: body.promptId }, "run fell back to another provider");
    }
    catch (e) {
    ...
    try {
        const data = parseStructured(raw, entry.schema);
...
app.post("/content/suggest", async (req, reply) => {
    ...
    const system = `You are HUMAIN Create Studio's content drafting agent. Propose a publish-ready FIRST DRAFT for a ` +
    ...
    try {
        const fb = await completeWithFallback({ system, messages: [{ role: "user", content: user }], fast: true, maxTokens: 1600 }, { primary: process.env.LLM_PROVIDER || "anthropic" });
        const raw = fb.text;
        if (fb.fellBack)
            req.log.warn({ used: fb.provider }, "content/suggest fell back");
        let data = {};
        ...
        if (m) {
            ...
            for (const f of body.fields)
                if (parsed[f.name] != null)
                    data[f.name] = String(parsed[f.name]);
        }
...
const PREVIEW_MODES = ["image", "deck"];
// Modes whose output is raw HTML rendered in a live in-app preview.
const HTML_MODES = ["websiteBuild"];
// Modes that produce long structured documents — give them more tokens.
const LONG_MODES = ["campaign", "brandGuideline", "websiteBuild", "video", "event", "webinar", "conference", "summit"];
const systemFor = {
    ...
    websiteBuild: "You are HUMAIN Create Studio, an expert front-end engineer. Build a COMPLETE, responsive, single-file landing page. " +
...
app.post("/studio/generate", async (req) => {
    const body = z
        ...
        .parse(req.body);
    const preview = PREVIEW_MODES.includes(body.mode);
    ...
    if (chosen.configured === false) {
        return {
            ...
            preview,
            artifact: `⚙️ ${entry.label} is not configured yet. Add the ${entry.provider.toUpperCase()} API key on the ai-service ` +
    ...
    try {
        ...
        if (fb.fellBack)
            req.log.warn({ tried: fb.tried, used: fb.provider, mode: body.mode }, "studio/generate fell back");
...
app.post("/studio/chat", async (req, reply) => {
    ...
    const send = (e) => reply.raw.write(`data: ${JSON.stringify(e)}\n\n`);
    ...
    const deliverableSpec = body.mode === "auto" || body.mode === "writing"
    ...
    try {
        await runOrchestrator({
            ...
            onEvent: (e) => { if (e.type === "reset")
                collected = ""; if (e.type === "artifact")
                streamed = true; send(e); },
        });
        send({ type: "done", model: entry.id, modelLabel: entry.label, artifact: collected });
    }
    catch (e) {
        const msg = e?.error?.error?.message || e?.message || "Generation failed.";
        const availability = /credit|balance|quota|rate.?limit|429|overload|insufficient|unavailable|timeout|econn|enotfound|fetch failed|throttl|capacity|\b5\d\d\b|529|premature|invalid response body|terminated|socket|reset|aborted|stream|connection|empty response|network/i.test(msg);
...
app.setErrorHandler((err, req, reply) => {
    ...
    if (!reply.sent)
        reply.code(sc === 400 ? 503 : sc).send({ ok: false, error: "service_error", detail: String(msg).slice(0, 300) });
});
